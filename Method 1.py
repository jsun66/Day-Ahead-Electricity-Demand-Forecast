...
df = pd.read_excel('Prep_Data.xlsx',sheet_name="Method_1_Train")
...
regressor.add(Dense(units = 4, kernel_initializer = 'glorot_uniform', activation = 'relu'))
...
forecast = pd.read_excel('Prep_Data.xlsx',sheet_name="Method_1_Forecast")
forecast.set_index('OPR_DATE',inplace = True)
X_forecast = forecast.iloc[:,0:3].values
X_forecast = sc_X.fit_transform(X_forecast)
y_forecast = sc_y.inverse_transform(regressor.predict(X_forecast))
y_real = forecast.iloc[:,4].values.reshape(-1,1)
